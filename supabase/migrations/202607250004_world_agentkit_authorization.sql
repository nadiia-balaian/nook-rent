create table nook.human_backed_authorizations (
  id uuid primary key default gen_random_uuid(),
  provider text not null check (provider = 'world_agentkit'),
  action text not null check (action = 'reservation_hold'),
  agent_address text not null check (length(agent_address) between 3 and 160),
  human_reference_hash text not null check (human_reference_hash ~ '^[0-9a-f]{64}$'),
  nonce text not null unique check (length(nonce) between 8 and 200),
  ...
  hold_id uuid unique references nook.reservation_holds(id),
  outcome text not null check (outcome in ('verified', 'hold_created', 'dates_unavailable')),
  created_at timestamptz not null default now()
);

alter table nook.reservation_holds
  add column human_backed_authorization_id uuid unique
    references nook.human_backed_authorizations(id);

create index human_backed_authorizations_active_limit_idx
  on nook.human_backed_authorizations (human_reference_hash, created_at desc);
...
begin
  ...
  if found then
    ...
    if existing_hold.listing_id <> p_listing_id
      ...
      or existing_authorization.nonce <> p_nonce
    then
...
revoke all on table nook.human_backed_authorizations from public;
...
alter table nook.human_backed_authorizations enable row level security;
